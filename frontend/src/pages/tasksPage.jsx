  ...
  const label = () => {
    ...
    return (
      <span className="text-lg font-bold text-gray-800 capitalize">
        {date.format("MMMM")}{" "}
        <span className="text-gray-400 font-light">{date.format("YYYY")}</span>
      </span>
    );
  };
...
const CustomEvent = ({ event }) => {
  ...
  if (type === "medication")
    styles = "bg-red-50 text-red-700 border-l-4 border-red-500";
  else if (type === "event")
    styles = "bg-purple-50 text-purple-700 border-l-4 border-purple-500";
  else if (type === "note")
    styles = "bg-yellow-50 text-yellow-700 border-l-4 border-yellow-500";

  return (
    <div
      className={`text-xs px-2 py-1 rounded-r-md h-full w-full flex items-center gap-1 font-medium truncate ${styles}`}
    >
  ...
  const calendarEvents = useMemo(() => {
    return filteredTasks.map((task) => ({
      ...
      start: task.dueAt ? new Date(task.dueAt) : new Date(),
        ...
        ? moment(task.dueAt).add(1, "hour").toDate()
      ...
      <div className="max-w-6xl mx-auto">
        ...
        {viewMode === "list" && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            ...
            <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
              <p className="text-yellow-600 text-xs font-semibold uppercase">
                Pending
              </p>
          ...
          <div className="space-y-4">
            {loading ? (
              [1, 2, 3].map((i) => (
                <div
                  key={i}
                  className="bg-white h-24 rounded-xl animate-pulse shadow-sm"
                ></div>
              ))
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <span
                          className={`flex items-center gap-1 text-[10px] uppercase font-bold px-2 py-0.5 rounded-full border ${
                      ...
                      <div className="mt-3 flex items-center gap-4 text-sm text-gray-500">
                        {task.recipientId && (
                          ...
                          >
                            ...
                            <span>
                              For: {task.recipientId.firstName}{" "}
                              {task.recipientId.lastName}
                            </span>
                          </div>
                        )}
                      </div>
