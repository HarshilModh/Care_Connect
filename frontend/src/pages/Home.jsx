  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const allTasks = Object.values(tasksData).flatMap(
          (group) => group.tasks || []
        );
        settasks(allTasks);
        ...
        const recentTasks = allTasks
          ...
          .slice(0, 3)
          .map((task) => ({
            text: `${task.status === "completed" ? "Completed" : "New"} task: ${
              task.title
            }`,
            time: task.dueAt
      ...
      >
        ...
        >
          <div className="card card-pad shadow-lg hover:shadow-xl transition-shadow border-l-4 border-indigo-500">
            ...
            <Link
              to="/tasks"
              className="text-sm text-indigo-600 font-semibold mt-4 inline-flex items-center gap-1 hover:gap-2 transition-all"
            >
          ...
          <div className="card card-pad shadow-lg hover:shadow-xl transition-shadow border-l-4 border-violet-500">
            ...
            <Link
              to="/family-groups"
              className="text-sm text-violet-600 font-semibold mt-4 inline-flex items-center gap-1 hover:gap-2 transition-all"
            >
          ...
          <div className="card card-pad shadow-lg hover:shadow-xl transition-shadow border-l-4 border-fuchsia-500">
            ...
            <Link
              to="/chat"
              className="text-sm text-fuchsia-600 font-semibold mt-4 inline-flex items-center gap-1 hover:gap-2 transition-all"
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <motion.div variants={item} className="lg:col-span-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <Link
                to="/tasks/create"
                className="group p-4 bg-white border border-slate-200 rounded-xl shadow-sm hover:shadow-md hover:border-indigo-300 transition-all flex items-center gap-4"
              >
              ...
              <Link
                to="/createGroup"
                className="group p-4 bg-white border border-slate-200 rounded-xl shadow-sm hover:shadow-md hover:border-violet-300 transition-all flex items-center gap-4"
              >
