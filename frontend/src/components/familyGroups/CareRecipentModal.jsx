                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <h3 className="text-xl font-bold text-gray-900">
                                        {careRecipient.userId?.firstName} {careRecipient.userId?.lastName}
                                    </h3>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div className="p-3 bg-gray-50 rounded-xl border border-gray-100">
                                    ...
                                    <p className="font-semibold text-gray-900">
                                        {careRecipient.dob ? new Date(careRecipient.dob).toLocaleDateString() : "Not set"}
                                    </p>
                                </div>
                                <div className="p-3 bg-gray-50 rounded-xl border border-gray-100">
                                    <div className="flex items-center gap-2 text-gray-500 text-xs font-medium uppercase mb-1">
                                    ...
                                    <p className="font-semibold text-gray-900 truncate" title={careRecipient.primaryCondition}>
                                ...
                                <div className="space-y-2">
                                    {careRecipient.emergencyContacts?.map((contact, index) => (
                                        <div key={index} className="flex items-center justify-between p-3 bg-red-50 rounded-lg border border-red-100">
                                            ...
                                            <a href={`tel:${contact.phone}`} className="text-red-600 hover:text-red-700 font-medium text-sm flex items-center gap-1">
                                                <Phone className="w-3 h-3" /> {contact.phone}
                                            </a>
                                        </div>
                                    ))}
                                    {(!careRecipient.emergencyContacts || careRecipient.emergencyContacts.length === 0) && (
                                        <span className="text-sm text-gray-500 italic">No emergency contacts listed.</span>
                                    )}
                                </div>
                            </div>
                        </div>
