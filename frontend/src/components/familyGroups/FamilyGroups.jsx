  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const msg = err?.response?.data?.error ?? err?.message ?? "Failed to load groups"
        if (!mounted) return
        setError(msg)
      } finally {
        if (!mounted) return
  ...
  const filtered = useMemo(() => {
    ...
    return (familyGroups || [])
      .filter((g) => {
        ...
        if (!qlc) return true
        return (g.groupName ?? "").toLowerCase().includes(qlc) || (g.description ?? "").toLowerCase().includes(qlc)
      })
    ...
    <main className="page">
      <div className="container-n">
        <header className="text-center mb-8">
          ...
          <p className="section-sub">Manage groups you created. Add members, edit, or delete when needed.</p>
        </header>
        ...
        <div className="max-w-4xl mx-auto">
          ...
          {!loading && !error && filtered.length === 0 && (
            <div className="card p-6 text-center">
              <p className="text-slate-500">No groups found. Create your first group to get started.</p>
              <div className="mt-4">
                <button className="btn-primary" onClick={() => navigate("/createGroup")}>Create group</button>
              </div>
            </div>
          )}
          ...
          {!loading && !error && filtered.length > 0 && (
            <div className="grid gap-4">
              {filtered.map((group) => {
                ...
                return (
                  <div key={id} className="card p-4 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    <div className="min-w-0">
                      <div className="flex items-center gap-3">
                        <h2 className="text-lg font-semibold truncate">{group.groupName}</h2>
                        {group.isPublic && <span className="text-xs text-slate-500 px-2 py-0.5 rounded bg-gray-100">Public</span>}
                      </div>

                      {group.description && (
                        <p className="text-slate-600 mt-2 truncate">{group.description}</p>
                      )}
                    ...
                    <div className="flex items-center gap-2">
                      <button className="btn-ghost" onClick={() => navigate(`/group-members/${id}`)}>Members</button>
                      <button className="btn-ghost" onClick={() => handleEdit(id)}>Edit</button>
