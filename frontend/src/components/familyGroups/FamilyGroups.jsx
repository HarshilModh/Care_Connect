...
const FamilyGroups = () => {
  ...
  const normalizedGroups = familyGroups.map((g) => {
    ...
    return {
      ...
      onboarding: (membership.onboardingStatus && membership.onboardingStatus !== "not_required")
  ...
  return (
    <div className="min-h-screen bg-gray-50/50 py-10 px-4 sm:px-6">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
          <div>
            ...
            <p className="text-gray-500 mt-2 text-lg">Manage your care circles and memberships.</p>
          </div>
          <button
        ...
        {error ? (
          ...
          <div className="text-center py-20 bg-white rounded-3xl border border-dashed border-gray-300">
            ...
            <p className="text-gray-500 max-w-sm mx-auto mb-6">
              You haven't joined or created any family groups yet. Get started by creating one.
            </p>
            <button onClick={() => navigate("/createGroup")} className="text-indigo-600 font-medium hover:underline">
              Create a new group
            </button>
          </div>
        ) : (
            ...
            {filteredGroups.map((group) => (
              ...
              >
                <div className="p-6 pb-4">
                      ...
                      <div className="relative">
                        ...
                        {activeDropdown === group.id && (
                          <div className="absolute right-0 top-full mt-2 w-48 bg-white rounded-xl shadow-xl border border-gray-100 py-1.5 z-20 animate-in fade-in zoom-in-95 duration-100">
                            <button
                              onClick={(e) => { e.stopPropagation(); handleEdit(group.id); }}
                  ...
                  <h3 className="text-xl font-bold text-gray-900 mb-2 truncate" title={group.name}>
                    {group.name}
                  </h3>
                ...
                <div className="mt-auto p-6 pt-0">
                  <div className="pt-4 border-t border-gray-100 flex flex-col gap-2">
                    ...
                    {(['careGiver', 'careRecipient'].includes(group.role) && group.onboarding === 'required') ? (
                      <button
                        onClick={() => {
                          if (group.role === "careGiver") navigate("/onboarding/caregiver");
                          else navigate(`/onboarding/carerecipient/${group.id}`);
                        }}
      ...
      <ToastContainer position="top-right" autoClose={3000} hideProgressBar={false} newestOnTop={false} closeOnClick rtl={false} pauseOnFocusLoss draggable pauseOnHover theme="colored" style={{ zIndex: 9999 }} />
    </div>
  );
};
