...
router.post("/", async (req, res) => {
    try {
        let {groupId,recipientId,createdBy,title,description,assignedTo,dueAt,timezone,repeatRule,type,notificationConfig,attachments,status} = req.body;
        if(!groupId ){
        ...
        const [creatorMember,recipientMember,assigneeMember] = await Promise.all([
            Membership.findOne({groupId: groupId, userId: createdBy, status: "active"}),
            Membership.findOne({groupId: groupId, userId: recipientId, status: "active"}),
            assignedTo ? Membership.findOne({groupId: groupId, userId: assignedTo, status: "active"}) : null
        ]);
        if(!creatorMember){
            return res.status(403).json({ error: "Creator is not an active member of the group" });
        }
        if(!recipientMember){
            return res.status(403).json({ error: "Recipient is not an active member of the group" });
        }
        if(assignedTo && !assigneeMember){
            return res.status(403).json({ error: "Assignee is not an active member of the group" });
        }
        //create task
        const newTask = await createTask(groupId,recipientId,createdBy,title,description,assignedTo,dueAt,timezone,repeatRule,type,notificationConfig,attachments,status);
...
router.put("/:taskId", async (req, res) => {
    try {
        let taskId = req.params.taskId;
        let {groupId,recipientId,createdBy,title,description,assignedTo,dueAt,timezone,repeatRule,type,notificationConfig,attachments} = req.body;
        if(!taskId){
            return res.status(400).json({ error: "Task ID is required" });
        }
        if(!groupId ){
      ...
      if(!mongoose.Types.ObjectId.isValid(taskId)){
        ...
        //update task
        const updatedTask = await updateTask(taskId,groupId,recipientId,createdBy,title,description,assignedTo,dueAt,timezone,repeatRule,type,notificationConfig,attachments);
...
router.post("/:taskId/complete", async (req, res) => {
    try {
        ...
        const member = await Membership.findOne({groupId: groupId, userId: completedBy, status: "active"});
        if(!member){
            return res.status(403).json({ error: "User is not an active member of the group" });
        }
...
router.post("/:taskId/reassign", async (req, res) => {
    try {
        ...
        const member = await Membership.findOne({groupId: taskExists.groupId, userId: newAssignee, status: "active"});
        if(!member){
            return res.status(403).json({ error: "User is not an active member of the group" });
        }   
