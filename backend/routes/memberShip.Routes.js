    ...
    for (const membership of memberships) {
      ...
      if (!groupId || !userId) {
        return res
          .status(400)
          .json({
            error: "Group ID and User ID are required for each membership",
          });
      }
      if (!mongoose.Types.ObjectId.isValid(groupId)) {
        return res.status(400).json({ error: "Invalid Group ID" });
      ...
      if (permissions && typeof permissions !== "object") {
        return res
          .status(400)
          .json({
            error: "Permissions must be an object in one of the memberships",
          });
      }
      //check if user is already in the group
      const isMember = await Membership.findOne({ groupId, userId });
      if (isMember) {
        return res
          .status(400)
          .json({
            error: `User ${userId} is already a member of group ${groupId}`,
          });
      }
...
router.put("/:membershipId/role", requireAuth, async (req, res) => {
  try {
    ...
    if (
      ...
      return res
        .status(400)
        .json({ error: "Role must be one of owner, caregiver, family" });
    }
...
router.put("/:membershipId/status", requireAuth, async (req, res) => {
  try {
    ...
    if (!["active", "pending", "removed"].includes(status)) {
      return res
        .status(400)
        .json({ error: "Status must be one of active, pending, removed" });
    }
